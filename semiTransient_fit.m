function [c_result] = semiTransient_fit(x_fit,S,unifac,mn_cell,Cb_0,t_exp,tf,h, ...
...
% return the values of cocnentration in t_exp time
% Input:
...
c_result = zeros(length(t_exp),1);
for i = 1:length(t_exp)
    idx = time==t_exp(i);
    c_result(i) = c_bulk(idx,p);
end
