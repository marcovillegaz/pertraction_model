...
for i = 1:length(names)
    ...
    ylabel('Concecentracion ug/mL','FontSize',10)
    ...
    flux = flux.*3600.*MW_pcb.*10
    D.both = D_fick_aq
    
    % experimental flux
    N1 = @(t) (-2*3.155e-10*t + 6.205e-09)*(vol_ext/At)*10*MW_pcb
    t_exp = 0:7
    
    hold on, grid on
    xlabel('Tiempo')
    ylabel('Flujo de transferencia de masa kg/m^2 s')

    plot(t_model(1:end-1),flux, ...
        'DisplayName','Modelo hibrido','LineWidth',1)

    plot(t_exp,N1(t_exp),'s', ...
        'DisplayName',"Experimental", ...
        'MarkerFaceColor',"#EB671B", ...
    ...
    legend('Extractant phase','Aqueous phase', ...
        'FontSize',12)
end
