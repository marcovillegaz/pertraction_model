function [c_result] = semiTransient_fit_both(x_fit,S,unifac,mn_cell,Cb_0,t_exp,tf,h, ...
    T,w_poly,cteEq,vol_phases,At,l)
% This function employ the finite difference in the Fick's second law to
% calculate the concetration in both phases of a perstraction process and
% return the values of concentration in t_exp time
%
% Input:
%   x_fit: row array where the the unknown variables are grouped.
%       ex: x_fit = [aij,xi,FVP,cteEq]
%   S: struct where de propeties of every compound are organized in fields
%   unifac: struct where unifac data are saved in groups 'Aij' and 'groups'
%   mn_cell: cell that contains postions of the aij unknowns in unifac.Aij
%   Cb_0: array with the molar concentration of both phases [aq ext] at t = 0
...
%   cteEq: row array with the equilibria constant of phases [eqAq eqExt]
%   vol_phases: row array with the volume of each phase [cm3] [Aq Ext]
%   At,l: membrane area an thickess [cm2] [cm]
...
if length(time) < length(t_exp)
    error("the simulated time is less than experimental time.\nTry another valuo of h")
end
...
% Fick's diffusion coefficient using Maxwell-Stefan model (assumed constamt)
...
c_bulk = zeros(length(time),2);      % bulk concentration [mol/cm3] 
flux = zeros(length(time)-1,1);      % molar flux [mol/cm2 s]  
delta_x = zeros(length(time)-1,1);   % molar gradient [-] 

%% --- SEMI TRANSIENT MDEOL ---
% concentration at t = 0
c_bulk(1,:) = Cb_0;  % this is experimental

for i = 1:length(time)-1 % time loop
    % Set old molar concentration
    c_bulk_old = c_bulk(i,:);
    
    % Concentration in interphase of both phases (equilibria) assumes
    % linear behavior (in the future could be add a subrutine to solve the
    % equilibria stage). 

    % cteEq = ci_poly [mol/cm3] / ci_aq [mol/cm3]
    c_inter = c_bulk_old.*cteEq;
    
    % new function: molar2weight() and weight2molar()
    ...
    % molar fraction in interphase (polymer side) (weight2molar)
...
c_result = zeros(length(t_exp),2);
for i = 1:length(t_exp)
    idx = time==t_exp(i);
    c_result(i,:) = c_bulk(idx,:);
end

c_result = [c_result(:,1);c_result(:,2)]; % *
